...
var res = await fetch("https://pharmabag.in:3000/seller/auth/orders/seller",{"headers" : {"content-type":"application/json","auth-token":localStorage.getItem("auth-token")} });
if(res.status==401){
...
milestone+=` <div class="col-12 col-sm-6">
                                                ...
                                                <input type="number" name="milestone" data-index="${l}" data-total="${data[i].cart_details.quantity}" data-value='${milestonequantity[l]}' value="${milestonequantity[l]}"  max="${data[i].cart_details.quantity}" class="form-control" onblur="if(this.value==0||this.value=''){this.value='';byId('dg${i}sa${l}').value='';return}">
                                            </div>
     ...
     model+=`<div class="modal fade" id="upload-invoice${i}">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form>
                                            <input id="" onchange="upload(this,'${data[i]._id}','${i}')" type="file"  >
...
${data[i].order_status=="divided"?`      
    ...
    html+=`
     ...
     <tr>
                                    ...
                                    <td> 
                                     <a onclick="getinvoices('${data[i]._id}','upload-invoice${i}')" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#upload-invoice${i}">Upload</a>  </td>
...
// async function setmilestone(formid,oid){


// var temp0=document.getElementById("setmilestoneform"+formid)
// num=Array.from(temp0.elements ).filter((cc)=>{return cc.type=="number"}).filter((e)=>e.value!="")
...
//            document.getElementById("error"+formid).innerText="Dates cannot be same! change date"
// wrongcount++
...
// wrongcount++
// document.getElementById("error"+formid).innerText="Dates cannot be empty! enter a date"
// }
// }catch(e){}
// }
...
//  var x=await fetch("https://pharmabag.in:3000/seller/auth/orders/seller/setmilestone/order/"+oid,{method:"POST",headers:{"content-type":"application/json","auth-token":localStorage.getItem("auth-token")},body:JSON.stringify({milestone_details:body,ispartial:0})});
//  if(x.status==200){
...
//==================
                     <td> ₹${["PAID","way to warehouse","sucessfull","Reached warehouse","Order in Transit","Out for Delivery","Order Shipped","Transport delay"].includes(data[i].order_status)?data[i].cart_details.pricing.final_order_value:data[i].cart_details.pricing.final_order_value-data[i].order_payment_details?.due} </td>
                     //==============================
...
for (var i = 0; i < imgdata.length; i++) {
    ...
    //=================================
    var progressBar = document.createElement("iframe");
...
for (var i = 0; i < imgdata.length; i++) {
 event=document.getElementById(`${imgdata[i].name.hashCode()}`)
    console.log(`${URL.createObjectURL(imgdata[i])}`)
        ...
        alert("Upload another image we cannot process this image Please Note Maximum file size is 5MB and allowed file types are ['pdf']")
        btn.remove()
        progressBar.remove()
        event.value=''
...
return new Promise((resolve, reject) => {
                    ...
                    let allowed_mime_types = ['application/pdf','image/jpeg','image/svg+xml', 'image/png','image/svg','image/webp','image/jpg'];
