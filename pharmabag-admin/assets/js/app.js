...
document.getElementById("notificatiocount").innerText=data.length>10?"10+":data.length
var html=''
...
async function checkAndDisplayNotifications() {
    if (location.href.includes("seller")||location.href.includes("buyer")) { return; }

    try {
        const lastCheckedTime = localStorage.getItem("lastCheckedTime");
        const currentTime = new Date().toISOString();

        const response = await fetch(`https://pharmabag.in:3000/admin/get/my/notification`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
        ...
        const newNotifications = notifications.filter(notification => {
            ...
            return notificationDate > new Date(lastCheckedTime);
        });

        if (newNotifications.length > 0) {
            localStorage.setItem("lastCheckedTime", currentTime);
           // document.getElementById("notifyId").innerText = newNotifications.length;
            document.getElementById("notifyId").style.display = "block";
            document.getElementById("notifyId").style.backgroundColor = "red";
            document.getElementById("notifyId").style.color = "white";
...
function displayNotifications(notifications) {
    ...
    popupContainer.style.right = "20px"; // Adjust this value to position the popups on the right side
    popupContainer.style.display = "flex";
    popupContainer.style.flexDirection = "column-reverse"; // Stack popups vertically from bottom to top
    popupContainer.style.alignItems = "flex-end";
    popupContainer.style.zIndex = "1000";
    popupContainer.style.overflowY="scroll"
    popupContainer.style.maxHeight = "60dvh";
    const popupStyles = {
        backgroundColor: "#5467ED",
	color:"white",
    ...
    notifications.forEach(notification => {
        ...
        popup.addEventListener("click", () => {
            popup.remove(); // Remove the popup when clicked
            document.getElementById("notifyId").style.display = "none";
        });

        popup.addEventListener("mouseover", () => {
            Object.assign(popup.style, popupHoverStyles);
        });
    ...
    // Automatically remove popups after a certain time
    setTimeout(() => {
...
// Call the function to check for new notifications
checkAndDisplayNotifications();
setInterval(()=>{checkAndDisplayNotifications();},10000)
document.getElementById("notifyId").style.display = "none";
