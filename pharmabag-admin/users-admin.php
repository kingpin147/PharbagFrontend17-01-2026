...
<div class="wrapper">
<input type="text" id="pcode" hidden>
<script>
   ...
   var offset=0; 
            ...
            async function _fdata(ofset){
                var x=await fetch("https://pharmabag.in:3000/admin/all/admin/"+ofset,{headers:{"Authorization":"<?php echo $_SESSION['token'];?>"}});
                var data= await x.json()
          
                return data
              }
            async function _data23(uid,wpid){
                var x=await fetch(`https://pharmabag.in:3000/user/is/in/wishlist/${uid}/${wpid}`);
                var data= await x.json()
                return data
              }
              
             _fdata(offset).then(async(data)=>{
               console.log(data)
              ...
              if(data[i]){
                                ...
                                <td class="">${data[i].admin_phone}</td>

                                <td class="">${data[i].admin_password}</td>
                                <td> 
  
  <span class='badge bg-success'>${data[i].admin_role}</span>
  </td>
                                <td>
...
<input type="text" class="dropdown-item" placeholder="Enter Permission code" style="border:2px solid black;margin-bottom:2px;" onkeyup="document.getElementById('pcode').value=this.value">
<button class="btn btn-primary" onclick="updaterole('${data[i]._id}')">Add</button>

                                        </ul>

                                        <a onclick="deleteuser('${data[i]._id}')" class="text-danger"><i class="bi bi-trash-fill"></i></a>
...
document.getElementById("tbody").appendChild(div)
    ...
    var x=await fetch("https://pharmabag.in:3000/admin/admin/status/reject/"+id,{method:"POST",headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"}});
               if(x.status==200){location.reload()}
                
    }
    async function pending(id){
    var x=await fetch("https://pharmabag.in:3000/admin/admin/status/pending/"+id,{method:"POST",headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"}});
               if(x.status==200){location.reload()}
    }
  
  
    async function accept(id){
    var x=await fetch("https://pharmabag.in:3000/admin/admin/status/accept/"+id,{method:"POST",headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"}});
               if(x.status==200){location.reload()}
                
    }
    async function sustemp(id){
    var x=await fetch("https://pharmabag.in:3000/admin/admin/status/sustemp/"+id,{method:"POST",headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"}});
               if(x.status==200){location.reload()}
    }
  
      async function deleteuser(id){
    var x=await fetch("https://pharmabag.in:3000/admin/admin/status/delete/"+id,{method:"POST",headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"}});
               if(x.status==200){location.reload()}
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex align-items-center">
                    ...
                    <form id="newadmin" onsubmit="event.preventDefault()" class="ms-auto position-relative">
 Name <input class="form-control ps-2" type="text" name="admin_name">
 Phone <input class="form-control ps-2" type="number" name="admin_phone">
 
 Password <input class="form-control ps-2" type="text" name="admin_password">
<button class="btn btn-primary" onclick="addadmin()">Add New Admin</button>
            ...
            <div class="container mt-4">
                ...
                <ul class="pagination ticket-page" style="justify-content: flex-end">
                    ...
                    <li class="page-item"><a onclick="loadmore()" class="page-link"> Load more </a></li>
    ...
    <div class="modal fade" id="approve-user">
...
var x=await fetch("https://pharmabag.in:3000/admin/create/admin",{method:"POST",headers:{"Authorization":"<?php echo $_SESSION['token'];?>","content-type":"application/json"},body:JSON.stringify(Form2JSON(document.getElementById("newadmin")))})
var data=await x.json()
                         ...
                         var div=document.createElement("tr")
                                ...
                                <td class="">${data.admin_phone}</td>
                                <td class="">${data.admin_password}</td>
                                <td> 
  
  ${
                 data.admin_status== 1 ? "<span class='badge bg-success'>Approved</span>"
                                :data.admin_status == 3 ? "<span class='badge bg-info'>Suspend Temporary</span>"
                                :data.admin_status == 2  ? "<span class='badge bg-danger'>Suspend Permanently</span>"                                     :"<span class='badge bg-warning'>Pending Review</span>"               
...
<input type="text" class="dropdown-item" placeholder="Enter Permission code" style="border:2px solid black;margin-bottom:2px;" onkeyup="document.getElementById('pcode').value=this.value">
<button class="btn btn-primary" onclick="updaterole('${data._id}')">Add</button>

                                        </ul>

                                        <a onclick="deleteuser('${data._id}')" class="text-danger"><i class="bi bi-trash-fill"></i></a>
...
document.getElementById("tbody").insertAdjacentElement('afterbegin', div)
...
var x=await fetch("https://pharmabag.in:3000/admin/edit/adminrole/"+id,{method:"POST",headers:{"Authorization":"<?php echo $_SESSION['token'];?>","content-type":"application/json"},body:JSON.stringify({"adminrole":document.getElementById("pcode").value})})
