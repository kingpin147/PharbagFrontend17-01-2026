  ...
  <style>
    ...
    <body>
        ...
        <div id="modal-container">
            ...
            <button id="closeModalButton" onclick="closeModal()" class="btn btn-primary">Close</button>
        </div>
    </div>
        <script type="text/javascript">0<th> Date </th>
                            <th> Product ID </th>
                            <th> Reason of cancellation </th>
 <td> ${new Date(data[i].date).toLocaleDateString()} </td>
                                    <td> ${data[i].product_id} </td>
                                    <td> ${data[i].message} </td></script>
<div class="wrapper">
...
<script type="text/javascript">
    // Function to fetch data from the API and format it nicely in the modal
async function fetchDataAndShowPopup(datax) {
  try {
    // Make an API request using the fetch function

    // Parse the response JSON
    console.log(decodeURI(datax))
    const data =JSON.parse(decodeURI(datax));

    // Create HTML content to display the data nicely
    const formattedData = `
      
      <p><strong>Order ID:</strong> ${data._id}</p>
      <p><a title="Product View"  target="_blank" class="fas fa-eye" href="users-buyer.php?id=${data.buyer_id}" tabindex="0"></a><strong>Buyer ID:</strong> ${data.buyer_id}</p>
      <p><a title="Product View" target="_blank" class="fas fa-eye" href="users-seller.php?id=${data.seller_id}" tabindex="0"></a><strong>Seller ID:</strong> ${data.seller_id}</p>
     
      <h3>Cart Details</h3>
      <p><strong>Product Name:</strong> ${data.cart_details.product_name}</p>
      <p><strong>Product Price:</strong> $${data.cart_details.product_price}</p>
      <!-- Add more properties as needed -->
    `;

    // Display the formatted HTML content in the modal
    document.getElementById("jsonContent").innerHTML = formattedData;
...
<script>
    (async()=>{
var res = await fetch("https://pharmabag.in:3000/admin/get/all/canceled/orders",{"headers" : {"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"} });
if(res.status==401){
...
for (var i = 0; i < data.length; i++) { 
    html+=`
     <tr>
 <td> ${new Date(data[i].date).toLocaleDateString()} </td>
                                    <td style="cursor:pointer;" onclick="fetchDataAndShowPopup('${encodeURI(data[i].product_id)}')" > <a title="Product View" class="fas fa-eye" tabindex="0"></a><small>view Details </td>
