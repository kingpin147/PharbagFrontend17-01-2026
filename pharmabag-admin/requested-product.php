...
$res = fetch("https://pharmabag.in:3000/user/get/all/suggestion?limit=100");
$xhtml='';
  $suggestion_json=json_encode($res->data);
...
const getAllTickets=async ()=>{
    var jsondata=JSON.parse(<?php echo json_encode($suggestion_json); ?>)
   ...
   html+=` <div class="col-12 alert-info mt-4">
                                            <div>
                                                ...
                                                <div class="orders">
                                                  ...
                                                  <a href="add_suggestion.php?name= ${data[i].name}&comp=${data[i].company_name}" class="mx-4" style="display:${jsondata.filter((elm)=>{return similarityRatio(elm.name,data[i].name)>=90}).length>0?'none':' '}">
