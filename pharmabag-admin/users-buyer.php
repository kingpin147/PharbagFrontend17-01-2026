...
<div class="popup" id="popupForm" style="display:none;">
     ...
     function createFormFields(form, data, prefix = "") {
            for (const key in data) {
                if (data.hasOwnProperty(key)) {
                    ...
                    if(['_id','date','__v','buyer_number_id','status',"buyer_id",'bank_account'].includes(fieldName)) continue
 ...
 if (typeof value != "object") {
        ...
        async function openPopup() {
            var x=await fetch("https://pharmabag.in:3000/admin/buyer/auth/profile/<?php echo $_GET['id']; ?>",{method:"GET",
    headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token']; ?>"}})
let sellerData=(await x.json()).buyer_details
 ...
 createFormFields(form, sellerData);
        ...
        document.getElementById("sellerForm").addEventListener("submit", async function (e) {
            ...
            var x=await fetch("https://pharmabag.in:3000/admin/buyer/auth/profile/edit/<?php echo $_GET['id']; ?>",{method:"POST",
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex align-items-center justify-content-around mt-3 gap-3">
                            <div class="text-center">
                                <h4 class="mb-0">${data.buyer_details.name}</h4>
                               <p class="mb-0"> Seller number id ${data.buyer_details.buyer_number_id}
                                   ${data.buyer_details.status!=0?"Active":"Pending"}
                                </p>
                                <p class="mt-2 text-secondary">Alternate ID: ${data.buyer_details.buyer_id}</p> 
                                <p class="mt-2 text-secondary">${data.buyer_details.legal_name}</p> <!-- business name-->
                            </div>
                        </div>
                        <!--                        business address-->
                        <div class="text-center mt-2">
                            <p class="mb-0 text-secondary">${data.buyer_details.address.street_address}</p>
                            <p class="mb-0 text-secondary">${data.buyer_details.address.street_address2}</p>
                        </div>
                        <hr>
                        <div class="text-start">
                        <button onclick="openPopup()">Edit Buyer Profile</button>
...
<li class="list-group-item d-flex justify-content-between align-items-center bg-transparent border-top">
                            Phone No.
                            <span class="">${data.buyer_details.phone_no}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center bg-transparent border-top">
                            Email
                            <span class="">${data.buyer_details.email}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center bg-transparent border-top">
                            PAN No.
                            <span class="">${data.buyer_details.gst_pan_response?.pan||data.buyer_details.gst_pan_response?.gstin.substring(2,12)}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center bg-transparent border-top">
                            GST No.
                            <span class="">${data.buyer_details.gst_pan_response?.gstin}</span>
                        </li>
                        
                        <li class="list-group-item d-flex justify-content-between align-items-center bg-transparent border-top">
                            Drug LIC 20B Licence
                            <span class="">${data.buyer_details.licence.DL20B[0]}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center bg-transparent border-top">
                            Valid Till
                            <span class="">${data.buyer_details.licence.DL20B[1]}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center bg-transparent border-top">
                            Drug LIC 21B Licence
                            <span class="">${data.buyer_details.licence.DL21B[0]}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center bg-transparent border-top">
                            Valid Till
                            <span class="">${data.buyer_details.licence.DL21B[1]}</span>
                        </li>
                    </ul>
            ...
            <div class="col-12 col-lg-7">
                <div class="card shadow-sm border-0">
                    <div class="card-body">
                        <h5 class="mb-0">${data.buyer_details.legal_name} Account Details Images</h5>
                        <hr>
                        <div class="card shadow-none border">
                            ...
                            <div class="card-body">
                                <img src="${data.buyer_details.licence.DL20B[2]}" alt="" width="100%">
                            </div>
                        </div>
                        <div class="card shadow-none border">
                            <div class="card-header">
                            ...
                            <div class="card-body">
                                <img src="${data.buyer_details.licence.DL21B[2]}" alt="" width="100%">
                            </div>
                        </div>
