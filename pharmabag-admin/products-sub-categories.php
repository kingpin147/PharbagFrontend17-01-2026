        ...
        <div class="card">
            ...
            <div class="card-body">
                            ...
                            <div class="card-body">
                                <form class="row g-3" onsubmit="event.preventDefault()">
                                    <div class="col-12">
                                        ...
                                        <input id="subcatadd" type="text" class="form-control" placeholder="sub Category name">
                                    </div>
                                    <div class="col-12">
                                        ...
                                        <select id="category" class="form-select" required>
                                            <script>
                                                var sclist={}
              const data=async()=>{var x=await fetch("https://pharmabag.in:3000/user/get/all/category");return x.json()}
              data().then(data=>{
               var html="<option>select category</option>"
              for(let i=0;i<data.length;i++){
              html+=`<option value=${data[i]["_id"]}>${data[i]["category_name"]}</option>`
              sclist[data[i]["_id"]]=data[i]["category_name"]
                                            }
              document.getElementById("category").innerHTML=html
                                    ...
                                    <div class="col-12">
                                        <div class="d-grid">
                                            <button class="btn btn-primary" onclick="addsubcat()">Add Sub Category</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
...
async function saveedit(element){
  ...
  var x=await fetch("https://pharmabag.in:3000/admin/edit/subcategory/"+element.parentElement.parentElement.parentElement.getElementsByTagName("td")[0].id,{
                  method:"POST",
                    headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"},
                    body:JSON.stringify({"subcategory_name":element.parentElement.parentElement.parentElement.getElementsByTagName("td")[0].innerText})
...
temp0.parentElement.appendChild(xxx)
     ...
     var x=await fetch("https://pharmabag.in:3000/user/get/all/subcategory",{
                  method:"GET",
 ...
 let tbl=document.createElement("tr")
                  tbl.innerHTML=` 
                                                                                                 <td id="${data[i]._id}">${data[i].subcategory_name}</td>
                                                <td>${sclist[data[i].parent]}</td>
                                                <td>
                                                    <div class="d-flex align-items-center gap-3 fs-6">
                                                     
                                                    <a onclick="initEdit(this)" class="text-warning"><i class="bi bi-pencil-fill"></i></a>
                                                        <a onclick="delete_subcat('${data[i]._id}')" class="text-danger"><i class="bi bi-trash-fill"></i></a>
                                                    </div>
                                                </td>`
                  
                  document.getElementById("subtbody").insertAdjacentElement('afterbegin', tbl)
      ...
      async function addsubcat(){
        var x=await fetch("https://pharmabag.in:3000/admin/add/subcategory",{
                  method:"POST",
                    headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"},
                    body:JSON.stringify({"name":document.getElementById("subcatadd").value,
                                         "parent":document.getElementById("category").options[document.getElementById("category").selectedIndex].value})
        ...
        tbl.innerHTML=` 
                                                <td id="${data._id}">${data.subcategory_name}</td>
                                                <td>${sclist[data.parent]}</td>
                                              
                                                <td>
                                                    <div class="d-flex align-items-center gap-3 fs-6">
                                                    <a onclick="initEdit(this)" class="text-warning"><i class="bi bi-pencil-fill"></i></a>
                                              <a onclick="delete_subcat('${data._id}')" class="text-danger"><i class="bi bi-trash-fill"></i></a> </div>
                                                </td>`
...
if(confirm("Are you sure to delete this subcategory?")){
       var x=await fetch("https://pharmabag.in:3000/admin/delete/subcategory/"+id,{
                  method:"POST",
                    headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"},
                    })
