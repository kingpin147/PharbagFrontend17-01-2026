    ...
    <main class="page-content">
        ...
        <div class="card">
            <div class="modal-body">
                <h6>Name: <span id="user_name"> Thomas Hardy </span></h6>
                <p>Code: <span id="code"> <?php echo $_GET['code']; ?></span> </p>
                <!-- <h5>Total Sale: ₹ <span id="total_sale"> 50000.00 </span></h5> -->
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="table-responsive mt-3">
                     <table class="table align-middle" id="tickets">
...
var res = await fetch("https://pharmabag.in:3000/admin/all/seller/"+offset,{"headers" : {"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"} });
if(res.status!=200){
...
var res = await fetch("https://pharmabag.in:3000/admin/get/referel/details/<?php echo $_GET['code']; ?>",{"headers" : {"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"} });
if(res.status!=200){
    alert("You must login as a admin,click ok to login")
    location.href="admin-signin.php"
}
var datax= await res.json()
document.getElementById("user_name").innerText=datax[0].user_name
...
for (var i=data.length-1;  i  >=0;  i--) { 
    var tr=document.createElement("tr")
     tr.innerHTML=`
                            ...
                            <td> ${data[i].seller_id} </td>
                            <td> ${data[i].legal_name} </td>
                            <td> ${data[i].name} </td>
                            <td> ${data[i].phone_no} </td>
                            <td> ${orders_count.find(x => x._id == data[i].seller_id)?orders_count.find(x => x._id == data[i].seller_id).count:"0"} </td>
                            <td>  <a href="sellerOrders.php?seller_id=${data[i].seller_id}" class="btn btn-primary">View</a> </td>
...
`
document.getElementById("ticket-summary2").appendChild(tr)
}
