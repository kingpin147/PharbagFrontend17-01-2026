            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Approved Buyer</li>
                    </ol>
                </nav>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="d-flex align-items-center">
                    ...
                    <form class="ms-auto position-relative">
                        ...
                        <input class="form-control ps-5" type="text" id="gfg" placeholder="search">
                    </form>
                </div>
...
<script>
    var checksellerlogin=()=>{}
    var offset=0
    const getorders =async(e)=>{
var res = await fetch("https://pharmabag.in:3000/admin/all/approved/buyer/"+offset,{"headers" : {"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"} });
if(res.status!=200){
...
case 1:
    order_status="Approved and Full payment"
    break;
case 2:
    order_status="Approved and EMI payment"
    break;
case 3:
    order_status="Approved and Full Credit"
    break;
case 0:
    order_status="Rejector keep in under-review"
    break;
...
default:
    ...
    var tr=document.createElement("tr")
     tr.innerHTML=`      <td>${data[i].buyer_number_id }</td>
      <td hidden>${data[i].buyer_id }</td>
                                <td>
                                    <div class="d-flex align-items-center gap-3 cursor-pointer">
                                        <img src="assets/images/avatars/avatar-1.png" class="rounded-circle" width="44" height="44" alt="">
                                        <div class="">
                                            <p class="mb-0">${data[i].legal_name}</p>
                                        </div>
                                    </div>
                                </td>
                          <td>${data[i].email} </td>

                                <td> <span class="pending">${order_status}</span> </td>
                                <td>${data[i].address.street_address} </td>
                                <td>${data[i].address.city}</td>
                                <td>${data[i].address.pincode}</td>
                                <td>India</td>
                                <td>
                                    <div class="table-actions d-flex align-items-center gap-3 fs-6">
                                        <a href="users-buyer.php?id=${data[i].buyer_id}" class="text-primary" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Views"><i class="bi bi-eye-fill"></i></a>
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                            <li><a class="dropdown-item" onclick="changestatus('${data[i].buyer_id}','1')">Approve and Full payment</a></li>
        <li><a class="dropdown-item" onclick="changestatus('${data[i].buyer_id}','2')">Approve and EMI payment</a></li>
        <li><a class="dropdown-item" onclick="changestatus('${data[i].buyer_id}','3')">Approve and Full Credit</a></li>
        <li><a class="dropdown-item" onclick="changestatus('${data[i].buyer_id}','0')">Reject or keep in under-review</a></li>
...
async function changestatus(buyer_id,type){

    var order_status=""
switch(type) {
case "1":
    order_status="Approve and Full payment"
    break;
case "2":
    order_status="Approve and EMI payment"
    break;
case "3":
    order_status="Approve and Full Due"
    break;
case "0":
    order_status="Rejector keep in under-review"
    break;
...
if (!confirm("Are you sure to mark this as "+order_status)){return}

 var x=await fetch("https://pharmabag.in:3000/admin/change/status/buyer",{method:"POST",headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"},body:JSON.stringify({
    status:type,
    buyer_id:buyer_id
...
location.reload()
    console.log(event.parentElement.parentElement)}
            ...
            <ul class="pagination" style="justify-content: flex-end">
              <li class="page-item page-prev"><a href="#" class="page-link" onclick="getorders(this)"> Load more </a></li>
  ...
  $(document).ready(function() {
                $("#gfg").on("keyup", function() {
                    ...
                    $("#ticket-summary tr").filter(function() {
