...
var res = await fetch("https://pharmabag.in:3000/admin/get/all/banar",{"headers" : {"content-type":"application/json","auth-token":localStorage.getItem("auth-token")} });
if(res.status==401){
...
html+=`<tr>  <td>
                                    <img src="${data[i].image}" alt="" width="60" height="60">
                                </td>
                                
                                <td>
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" ${data[i].active==1?"checked='true'":' '} onclick="update('${data[i]._id}',this)">
                                    </div>
                                </td>
                                <td>
                                    

                                    <a  class="text-danger" onclick="delete_subcat('${data[i]._id}')"><i class="bi bi-trash-fill"></i></a>
                                </td></tr>`

}
document.getElementById("banars").innerHTML=html
                    ...
                    <div class="modal fade" id="upload">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-12">
                                                <label class="form-label">Slider Image</label>
                                                  <input id="" onchange="upload(this)" type="file" accept="image/*,.pdf" multiple>
                        <div id="con" class="con" style="display: grid;grid-template-columns: repeat(auto-fit, minmax(200px, 3fr));grid-gap: 20px;margin-top: 20px;">
                                        ...
                                        <button type="button"  class="next btn btn-primary mt-3">Submit</button>
                                    </form>
                                </div>
                            </div>
                        </div>
...
// /admin/edit/banar/:id
    ...
    async function addsubcat(){
        var x=await fetch("https://pharmabag.in:3000/admin/add/banar",{
                  method:"POST",
                    headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"},
                    body:JSON.stringify({"image":buyer_licences_image[0],
                                         "link":document.getElementById("link").value})
                  
                  })

         if(x.status==403){alert("Login first!");location.href="admin-signin.php"}
        ...
        tbl.innerHTML=`  <td>
                                ...
                                <td>
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" ${data.active==1?"checked='true'":' '} onclick="update('${data._id}',this)">
                                    </div>
                                </td>
                                <td>
                                    

                                    <a  class="text-danger" onclick="delete_subcat('${data._id}')"><i class="bi bi-trash-fill"></i></a>
                                </td>`
        
         document.getElementById("banars").insertAdjacentElement('afterbegin', tbl)
      }
      
        
         async function delete_subcat(id){
if(confirm("Are you sure to delete this banar?")){
       var x=await fetch("https://pharmabag.in:3000/admin/delete/banar/"+id,{
                  method:"POST",
                    headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"},
                    })
       if(x.status==200){location.reload()}
        else if(x.status==403){alert("Login first!");location.href="admin-signin.php"}
...
if(confirm("Are you sure to change status of this banar?")){
       var x=await fetch("https://pharmabag.in:3000/admin/edit/banar/"+id,{
                  method:"POST",
                    headers:{"content-type":"application/json","Authorization":"<?php echo $_SESSION['token'];?>"},
                    body:JSON.stringify({active:event.checked?1:0})
                    })
       if(x.status==200){location.reload()}
        else if(x.status==403){alert("Login first!");location.href="admin-signin.php"}
