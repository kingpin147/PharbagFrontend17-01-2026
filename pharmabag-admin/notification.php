...
<div id="chatbox" style="height: 58vh;overflow: scroll;">
        ...
        <div class="input-group mt-4" id="reply">
            <input id="chat_input" type="text" class="form-control col-3" placeholder="type a Notifications" name="email" required>
            <button class="input-text" id="replay" onclick="sendmessage('everyone')">To all</button>
            <button class="input-text" style="background: #8bc73a;" id="closechat" onclick="sendmessage('buyer')">Buyer</button>
            <button class="input-text" style="" id="closechat" onclick="sendmessage('seller')">seller</button>
            <button class="input-text" style="" id="closechat" onclick="deleteall()">delete all</button>
        </div>
...
var x=await fetch(`https://pharmabag.in:3000/admin/get/notification`,{
...
if (x.status==401 || x.status==403) {
    ...
    alert("You must first login as a admin")
...
html+=`
    <div class="card">
            <div class="card-header">
                <div class="">
                    <p> <b> ${data[i].user_type} : </b> <span> ${new Date(data[i].date).toLocaleDateString()} </span> </p>
                </div>
                <div class="card-body">
                    <div class="">
                        <p>${data[i].message}</p>
...
document.getElementById("chatbox").innerHTML=data.length>0?html:`<center><h3>No message form anyone start some discussion</h3></center>`
...
var x=await fetch(`https://pharmabag.in:3000/admin/add/notification`,{
...
body:JSON.stringify({user_type:type,user_id:'everyone',message:ci.value})
...
var x=await fetch(`https://pharmabag.in:3000/admin/delete/notification`,{
...
var x=await fetch(`https://pharmabag.in:3000/admin/update/status/ticket/<?php echo $_GET["id"];?>`,{
