...
var res = await fetch("https://pharmabag.in:3000/admin/get/all/custom/orders",{"headers" : {"content-type":"application/json","Authorization":"<?php echo $_SESSION['token']; ?>"} });
if(res.status==401){
...
if(x.status==200){
    ...
    for (var i = data.invoice.length - 1; i >= 0; i--) {
        imgloop+=`<li><iframe src="${data.invoice[i]}" width="400px" height="300px"  />
</li>`
    }
